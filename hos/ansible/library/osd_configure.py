...
def _run_command(module, command, shell=False, strip_output=True,
                 ignore_error=False):
    '''
    Run a module command raising an exception if the command failed.
    :param command: the command to execute
    :shell: if True, calls subprocess.Popen with shell=True to include
                                              all shell based features
    ...
    '''
    rc, out, err = module.run_command(command, use_unsafe_shell=shell)
...
def create_and_configure_osd(module, hostname, fstype, data_disk, journal_disk,
    ...
    activate_cmd = 'ceph-disk -v activate %s --mark-init=%s %s' % (
        bootstrap_keyring, mark_init, data_disk)
    ...
    if persist_mountpoint == 'fstab':
        disk_uuid = get_osd_disk_uuid(module, mount_args[2])
        fstab_mount = "\n%s %s %s defaults,nofail,%s 0 2\n" % (disk_uuid,
                                                               mount_args[3],
                                                               fstype,
                                                               mount_args[1])
    ...
    _run_command(module, crushadd_cmd)
...
def get_osd_disk_uuid(module, data_disk):
    '''
    Returns disk UUID for the partitioned osd data disk
    :param data_disk: device to be used for object storage
    :return: UUID=<uuid> of the partitioned disk.
    '''
    uuid_cmd = "blkid %s | awk '{print $2}'" % data_disk
    uuid, err = _run_command(module, uuid_cmd, shell=True)
    if uuid[0:4] != "UUID":
        raise Exception("Failed to get data disk %s uuid" % data_disk)
    return uuid
    ...
    try:
        ...
        if retval == 0:
            ...
            retval = create_and_configure_osd(
                ...
                params['mark_init'], params['data_disk_poll_attempts'],
