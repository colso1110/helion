...
def search_node(node, parent_not=False):
    if isinstance(node, nodes.And):
        ...
        if (node.name == "defined" and node.node.name.startswith(HLM_PREFIX) and
           node.node.ctx == "load" and not parent_not):
            yield node.node.name


#
# This function processes the fact cache host entries to remove registered
# HLM_PREFIX variables marked for deletion, along with the deletion markers.
#
    ...
    for key in cache:
        try:
            for host_key in cache[key].keys():
                if (host_key.startswith(HLM_PREFIX) and host_key.endswith(".deleted") and
                   cache[key].get(host_key, "False")):
                    hlm_key = host_key.split(".deleted")[0]
                    del cache[key][host_key]
                    if hlm_key in cache[key]:
                        del cache[key][hlm_key]
        except Exception:
...
class CallbackModule(object):
    ...
    def runner_on_ok(self, host, res):
        ...
        if self.task:
            if self.task.register and self.task.register == "hlm_notify_set_persistent_facts_to_false":
                # Get hlm_notify variables from SETUP_CACHE and set changed to False
                update_dict = {}
                prefix_to_set_false = HLM_PREFIX
                if 'hlm_notify_set_false_prefix' in self.playbook.VARS_CACHE[host].keys():
                    prefix = self.playbook.VARS_CACHE[host].pop('hlm_notify_set_false_prefix')
                    prefix_to_set_false = prefix if prefix.startswith(HLM_PREFIX) else HLM_PREFIX
                for key in self.playbook.SETUP_CACHE[host].keys():
            ...
            elif self.task.register and self.task.register.startswith(HLM_PREFIX):
                ...
                if res.get('changed'):
                    ...
                    previous_state = self.playbook.SETUP_CACHE[my_host].get(self.task.register, None)
                    if previous_state:
            ...
            if self.task.when:
                if isinstance(self.task.when, list):
                    ...
                    for elem in self.task.when:
                        if HLM_PREFIX in elem:
                            when = elem
                else:
                    when = self.task.when if HLM_PREFIX in self.task.when else None
                if when:
